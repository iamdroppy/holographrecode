...
namespace Ion.HabboHotel.Habbos
{
    ...
    public class Habbo : IDataObject, ISerializableObject
    {
        ...
        private bool CheckoutUserParams(ref DataRow dRow)
        {
            ...
            mID = (uint)dRow["id"];
            mUsername = (string)dRow["name"];
            mPassword = (string)dRow["password"];
            mSignedUp = DateTime.Parse(dRow["hbirth"].ToString());

            mEmail = (string)dRow["email"];
            mDateOfBirth = (string)dRow["birth"];

            mMotto = (string)dRow["mission"];
            mFigure = (string)dRow["figure"];
            Gender = (((string)dRow["sex"] == "M") ? 'M' : 'F');

            mCoins = (uint)dRow["credits"];
            mFilms = 90001;
            mGameTickets = (uint)dRow["tickets"];
        ...
        public bool LoadBySsoTicket(DatabaseManager database, string sTicket)
        {
            ...
            using (DatabaseClient dbClient = database.GetClient())
            {
                ...
                result = dbClient.ReadDataRow("SELECT * FROM users WHERE ticket_sso = @ticket LIMIT 1;");
                if (result != null)
                {
                    // Prevent re-using of SSO ticket
                    dbClient.ExecuteQuery("UPDATE users SET ticket_sso = NULL WHERE ticket_sso = @ticket LIMIT 1;");
                }
                return CheckoutUserParams(ref result);
            }
        }
