        ...
        public void ReloadRights()
        {
            ...
            using (DatabaseClient dbClient = IonEnvironment.GetDatabase().GetClient())
            {
                ...
                for (byte role = 0; role <= mMaxRoles; role++)
                {
                    ...
                    DataTable result = dbClient.ReadDataTable("SELECT fuseright FROM system_fuserights WHERE minrank <= " + role.ToString() + ";");
                    if (result.Rows.Count > 0)
                    {
                        ...
                        {
                            rights[position] = (string)result.Rows[position]["fuseright"];
                        }
